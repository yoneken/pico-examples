...
int main() {
    ...
    PIO pio = pio0;
    uint sm = 0;
    uint offset = pio_add_program(pio, &rs422_tx_program);
    rs422_tx_program_init(pio, sm, offset, PIN_TX, SERIAL_BAUD);

    while (true) {
        rs422_tx_program_puts(pio, sm, "Hello, world! (from RS422!)\n");   // U:0x55
        sleep_ms(1000);
    }
}
